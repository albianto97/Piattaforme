...
def on_chat_message(msg):
	...
	# start command
		...
		msg ="Inserisci il nome della citta:"
		markup = ReplyKeyboardMarkup(keyboard=[["Bergamo", "Milano"], ["Monza", "Rho"], ["Mantova","Brescia", "Livigno"]])
       		bot.sendMessage(chat_id, msg, reply_markup=markup)	
		utente[chat_id] = 4
		...
		print(albergo+"   esperia    ")
		print(msg['text']+"   rho    ")
		try:	
			r = requests.get(
                		url='https://www.dati.lombardia.it/resource/7n8z-rsk5.json?denominazione_struttura='+albergo	+'&&nome_comune='+str(msg['text']))
                	json_data = r.json()
			#estraggo i dati JSON
			count=0
			for i in json_data[0:len(json_data[0])]:
